...
export default function PointOfSale() {
  ...
  const handleCustomerNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.length <= 50 && /^[a-zA-Z0-9\s]*$/.test(val)) {
      ...
      showToast("error", "Invalid Input", "Customer name cannot exceed 50 characters.");
    }
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      for (const item of cart) {
        ...
        if (!exactId) {
          showToast("error", "System Error", `ID missing for '${item.name}'. Transaction halted to prevent data corruption.`);
          setIsProcessing(false);
          return;
        }

        formattedCartItems.push({
          partId: Number(exactId),
      ...
      console.error("Checkout Error Payload:", err.response?.data || err.message);
  ...
  return (
    <div className="h-[calc(100vh-72px)] bg-slate-100 p-4 relative overflow-hidden">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col h-full overflow-hidden">
          <div className="p-5 border-b border-slate-100 bg-slate-50/50">
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm font-bold text-slate-800 transition-all shadow-sm"
            />
          </div>
          <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 md:grid-cols-3 gap-3 content-start">
            {filteredInventory.map(part => (
              <div
                key={part.partCode}
                onClick={() => part.currentStock > 0 && addToCart(part)}
                className={`relative p-4 rounded-xl border transition-all duration-200 select-none flex flex-col justify-between min-h-[130px] ${
                  part.currentStock > 0
                  ? 'bg-white border-slate-200 hover:border-blue-400 hover:shadow-lg hover:shadow-blue-500/5 hover:-translate-y-1 cursor-pointer'
              ...
              >
                <div>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {part.currentStock > 0 && part.currentStock <= 5 && (
                      <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" title="Low Stock"></span>
                    )}
                  </div>
                ...
                <div className="flex justify-between items-end mt-4">
                  ...
                  <div className={`text-[10px] font-bold px-2 py-1 rounded-md ${
                    part.currentStock > 10 ? 'bg-emerald-50 text-emerald-600' :
                    part.currentStock > 0 ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-600'
                  }`}>
        ...
        <div className="lg:col-span-1 bg-slate-900 rounded-2xl shadow-xl flex flex-col h-full overflow-hidden border border-slate-800">
          <div className="p-5 border-b border-slate-800 bg-slate-950">
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 text-white rounded-lg outline-none placeholder-slate-500 text-sm font-medium focus:border-blue-500 transition-all"
            />
          </div>
          ...
          <div className="p-4 bg-slate-950 border-t border-slate-800">
            <div className="flex justify-between items-end mb-4">
              ...
              <span className="text-2xl font-black text-white">Rs. {cartTotal.toLocaleString()}</span>
            </div>
            <button
